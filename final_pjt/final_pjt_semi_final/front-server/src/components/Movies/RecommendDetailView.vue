  ...
  <div>
    ...
    <div v-for="comment in comments" :key="comment.id">
      <b-card title="Comment" class="mb-2">
        ...
        <div>
          <button variant="primary" @click="toggleLike(comment.id)">
            {{ comment.liked ? '좋아요 취소' : '좋아요' }}
          </button>
          <p>좋아요 수: {{ comment.likes }}</p>
        </div>
...
export default {
  ...
  methods: {
      ...
      axios.get(`http://localhost:8000/movies/${movieId}/comments/`, { headers: {
        ...
        .then(response => {
          this.comments = response.data.map(comment => ({
            ...comment,
            likes: comment.likes || 0,  // likes 값이 존재하면 사용하고, 없다면 0으로 초기화
            liked: false
          }))
        })
        .catch(error => {
    ...
    toggleLike(commentId) {
      // 좋아요 상태를 서버에 업데이트
      axios.post(`http://localhost:8000/movies/${this.$route.params.id}/comments/${commentId}/like/`, {}, {
        headers: {
          'Authorization': `Token ${this.$store.state.token}`,
        },
      })
      .then(response => {
        const { follow, count } = response.data
        const commentIndex = this.comments.findIndex(comment => comment.id === commentId)
        // Update the comment with the server's response
        if (commentIndex !== -1) {
          this.$set(this.comments, commentIndex, {
    ...
    getLikes() {
      const movieId = this.$route.params.id;
      axios.get(`http://localhost:8000/movies/${movieId}/user_likes/`, { headers: {
        'Authorization': `Token ${this.$store.state.token}`,}})
        .then(response => {
          const likedCommentIds = response.data;
          this.comments.forEach(comment => {
            ...
            comment.likes = comment.like_users.length;
          });
        })
        .catch(error => {
          console.log(error);
